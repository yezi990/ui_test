...
async function apiPromise(){
	return new Promise(function(resolve,reject){
		callSpec1(function(error){
			if(error){
				console.log('[Request API ERROR] - ',error.message);
...
module.exports = function(){
    ...
    it('url: https://itest.clife.net/beadhouseEnter/#/login', async function(){
        await driver.url(_(`https://itest.clife.net/beadhouseEnter/#/login`));
    });
    ...
    callSpec('commons/login.js');

    it('click: #root li:nth-child(1) > div.middle > img.icon, 54, 84, 0', async function(){
        await driver.sleep(300).wait('#root li:nth-child(1) > div.middle > img.icon', 30000)
               .sleep(300).mouseMove(54, 84).click(0);
    });
    ...
    it('expect: text, //span[text()="证件类型"], equal, 证件类型', async function(){
        await driver.sleep(300).wait('//span[text()="证件类型"]', 30000)
            .text()
            .should.not.be.a('error')
            .should.equal(_(`证件类型`));
    });
	
	callSpec1('E:/ui_m/sample/web_login.js')
   /*
   it('调用接口获取accessToken', async function(){		
		await apiPromise().then(function(){
...
function callSpec1(){
    try{
        ...
        //require(name)();
		cp.execSync("E:/test13/testx.bat");
